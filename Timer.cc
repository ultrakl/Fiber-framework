    ...
    m_manager(manager)
{
    ...
    m_next = ts.tv_sec * 1000 + ts.tv_nsec / 1000000;
}
...
bool Timer::refresh() {
    ...
    timespec ts = {0};
    clock_gettime(CLOCK_MONOTONIC, &ts);
    m_next = ts.tv_sec * 1000 + ts.tv_nsec / 1000000 + m_ms;
    m_manager->m_timers.erase(it);
    return true;
}
...
TimerManager::TimerManager() {
    timespec ts = {0};
    clock_gettime(CLOCK_MONOTONIC, &ts);
    m_previouseTime = ts.tv_sec * 1000 + ts.tv_nsec / 1000000;
}
...
static void OnTimer(std::weak_ptr<void> weak_cond, std::function<void()> cb) {
    //原子性的构建一个共享指针，使用weak_ptr的expired再对原始指针解引用可能因多线程析构导致
    //原指针不能使用
